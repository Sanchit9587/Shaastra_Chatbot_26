...
# Import New Modules
from query_transform import create_query_rewriter
...
class GraphState(TypedDict):
    ...
    standalone_question: str # The rewritten question (for retrieval)
    generation: str
    documents: List[Any]
...
grader_chain = create_grader(llm)
...
def format_history(history):
    # Get last 6 lines (3 turns)
    return "\n".join(history[-6:]) 

def print_retrieved_chunks(documents):
    print("\n" + "="*40)
    print(f"🔍 DEBUG: Reranker kept top {len(documents)} Context Chunks")
    print("="*40)
...
def route_query(state):
    ...
    # 1. Check for Contextual Dependency (Follow-up)
    # If question contains pronouns, route to rewriter
    pronouns = ['it', 'that', 'this', 'he', 'she', 'they', 'those', 'there']
    if any(word in question.lower().split() for word in pronouns):
        print("---DECISION: FOLLOWUP (Needs Rewrite)---")
        return {"intent": "followup"}

    # 2. Standard Router Prompt
...
2. Return 'general' ONLY if the question is:
   ...
   - Completely unrelated (e.g., "Who is the President?").
...
def rewrite_query(state):
    """
    Rewrites questions like "When does it start?" -> "When does Robowars start?"
    """
    ...
    standalone = rewriter_chain.invoke({"history": history, "question": question})
    # Cleaning any preamble
...
def retrieve(state):
    ...
    # Use standalone if available, else original
...
def generate_rag(state):
    ...
    question = state["question"] # Use original question for tone
    documents = state["documents"]
    history_str = format_history(state["chat_history"])
    
    if not documents:
        return {"generation": "I looked through the Shaastra documents but couldn't find specific information regarding that."}
...
def grade_generation(state):
    ...
    if not documents: return {"generation": generation}
    
    # Flatten context for grader
    context_text = "\n".join([d.page_content for d in documents])
    
    score = grader_chain.invoke({"documents": context_text, "generation": generation})
    
    if "no" in score.lower():
        print("❌ GRADER: Hallucination Detected.")
        return {"generation": "I apologize, but I could not find verified information in the Shaastra documents to answer that specific question accurately."}
    else:
        print("✅ GRADER: Answer Validated.")
        return {"generation": generation}
...
workflow.add_node("grader", grade_generation)
...
workflow.add_edge("generate_rag", "grader") # Check for hallucinations
...
if __name__ == "__main__":
    ...
    while True:
        try:
            ...
            # Strip Chain of Thought steps if they leaked
            if "Step 3" in clean_ans: clean_ans = clean_ans.split("Step 3")[-1].strip()
